...
try {
  const emailResponse = await fetch('/api/send-adoption-email', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      ...
      package_name: selected.frame_code,
      donation_amount: formatMvrAmount(selected.donation_amount),
  ...
  if (!emailResponse.ok) {
    ...
    console.error('Adoption email send failed:', emailResult);
  }
} catch (emailError) {
  console.error('Adoption email request failed:', emailError);
}
    ...
    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
      <div>
        ...
        <p className="mt-3 text-slate-600 max-w-2xl">
          Select a one-time coral frame contribution package below and the adoption
          form will open for that package.
        </p>
      </div>
